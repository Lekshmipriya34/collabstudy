...
function FlashcardDeck({ collectionPath, title }) {
  ...
  // 1. Fetch Cards from the given path (Private or Room)
  useEffect(() => {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-[2rem] shadow-xl border border-purple-50 p-6 flex flex-col h-[400px]">
        ...
        {isAdding ? (
          ...
          <form onSubmit={handleAdd} className="flex flex-col gap-3 h-full justify-center">
            <input
              ...
              className="p-3 rounded-xl border border-slate-200 focus:border-purple-400 outline-none text-sm font-medium text-black"
            />
            <textarea
              placeholder="Back (Answer)"
              value={back}
              onChange={(e) => setBack(e.target.value)}
              className="p-3 rounded-xl border border-slate-200 focus:border-purple-400 outline-none text-sm font-medium h-24 resize-none text-black"
            />
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-2xl">
            ...
            <p className="text-xs">Click "+ Add New" to start</p>
          </div>
        ) : (
          // === CARD VIEW (FLIP ANIMATION) ===
          <div
          ...
          >
            {/* FRONT */}
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center p-6 text-center shadow-lg [backface-visibility:hidden]">
              <div>
                <span className="absolute top-4 left-4 text-[10px] text-white/50 font-black uppercase tracking-widest">Question</span>
                <p className="text-white text-xl font-bold leading-relaxed">
                  {cards[currentIndex]?.front}
                </p>
                <p className="absolute bottom-4 right-4 text-xs text-white/50 animate-pulse">
                  Click to flip ↻
                </p>
              </div>
            </div>
