  ...
  useEffect(() => {
    const fetchUniversalResources = async () => {
      ...
      try {
        ...
        // 2. Fetch resources from the top-level "resources" collection for each room
        const fetchPromises = roomsSnap.docs.map(async (roomDoc) => {
          const roomData = roomDoc.data();
          
          // FIXED: Query the top-level 'resources' collection using the roomId
        ...
        // FIXED: Properly convert Firestore Timestamps to milliseconds for sorting
        const combinedResources = results.flat().sort((a, b) => {
  ...
  if (loading) {
    return (
      <div className="glass-card p-6 flex justify-center items-center h-40">
        <p className="text-purple-200 animate-pulse font-bold">Loading Library...</p>
      </div>
    );
  }
